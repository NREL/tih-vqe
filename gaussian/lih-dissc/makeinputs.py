...
intemp = ['%chk=molecule\n','#P CCSD(MaxCyc=200,SaveAmplitudes,ReadAmplitudes)/GEN sp pop=AllOrbitals iop(3/33=1,3/36=-1)\n',
          '\n','test\n','\n',
          'mult',
          'H 0.0 0.0 0.0\n',
          'placeholder',
...
for d in dists:
    for m in mults[a]:
        for ib,b in enumerate(basissets):            
            infile = [x for x in intemp]
            infile[5] = '0 ' + str(m) + '\n'
            infile[7] = a + ' 0.0 0.0 ' + str(d) + '\n'
            infile[12] = a + ' 0\n'
            infile[13] = b + '\n'
            ...
            if not os.path.isdir(dirname):
                os.makedirs(dirname)
            writefile(dirname+'/in',infile)
            copy2('submit.log2',dirname)
    temp += str(round(d,1))+' '
print(temp) 
